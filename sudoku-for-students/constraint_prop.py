...
def AC3(csp, queue=None, removals=None):
    ...
    # initialize queue with variables and neighbors
    if queue == None:
        queue = []
    ...
    # while q is not empty, use revise to check if domains violate constraints
    q = queue
    while q:
        Xi,Xj = q.pop(0)    # grab value from queue
        if revise(csp, Xi, Xj, removals):
            if csp.curr_domains[Xi] == []:   # if variables domain is reduced to empty list, return false
                return False
            else:         # otherwise, populate queue
...
def revise(csp, Xi, Xj, removals):
    ...
    """
    revised = False
    for x in csp.curr_domains[Xi]:  # iterate through domain of Xi
        if all(element == x for element in csp.curr_domains[Xj]):   # if Xj has been assigned and is in the domain of Xi, prune that value from Xi
